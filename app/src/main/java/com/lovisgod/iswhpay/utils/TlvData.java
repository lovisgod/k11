...
public class TlvData {
    ...
    private byte[] value;
    private static final String TAG = "TlvData";
    ...
    public static TlvData fromRawData(byte[] tlData, int tlOffset, byte[] vData, int vOffset) {
        if (tlData == null || tlData.length == 0 || vData == null || vData.length == 0) {
            return null;
        }
    ...
    public static TlvData fromData(String tagName, byte[] value) {
        if (tagName == null || tagName.length() == 0 || value == null || value.length == 0) {
            return null;
        }
    ...
    private static int calcValueLength(byte[] l, int offset, int lLen) {
        ...
        Log.d(TAG, "calcValueLength: "+l.length);
